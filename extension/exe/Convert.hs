...
parseCall :: Parser Element Program
parseCall e = do
    v <- field "NAME" (elementNodes e)
    n <- parseNext e
    return $ CallStmt (unpack v) : n

parseFn :: Parser Element Program
parseFn e = do
    name <- field "NAME" (elementNodes e)
    body <- statement "DO" e
    n <- parseNext e
    return $ FnStmt (unpack name) body : n
...
parseExprTy ty                = const $ throwE $
    "Unknown block expr type: " ++ unpack ty
...
-- Function stuff.
parseStmtTy "call"                = parseCall
parseStmtTy "fn"                  = parseFn
parseStmtTy ty = const $ throwE $
    "Unknown block stmt type: " ++ unpack ty
